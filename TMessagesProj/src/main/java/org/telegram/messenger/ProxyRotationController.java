...
public class ProxyRotationController implements NotificationCenter.NotificationCenterDelegate {
    ...
    private void switchToProxy(SharedConfig.ProxyInfo info, String reason) {
        SharedConfig.ProxyInfo selectedInfo = null;
        ...
        for (SharedConfig.ProxyInfo currentCandidate : sortedList) {
            if (currentCandidate == SharedConfig.currentProxy || currentCandidate.checking || !currentCandidate.available || currentCandidate.proxyType == SharedConfig.ProxyInfo.PROXY_TYPE_XRAY_VLESS) {
                continue;
            }
            selectedInfo = currentCandidate;
            break;
        }

        if (selectedInfo == null) {
            log("no available proxy to switch");
            return;
        }

        engine.recordSwitch(selectedInfo);
        SharedPreferences.Editor editor = MessagesController.getGlobalMainSettings().edit();
        editor.putString("proxy_ip", selectedInfo.address);
        editor.putString("proxy_pass", selectedInfo.password);
        editor.putString("proxy_user", selectedInfo.username);
        editor.putInt("proxy_port", selectedInfo.port);
        editor.putString("proxy_secret", selectedInfo.secret);
        editor.putBoolean("proxy_enabled", true);

        if (!selectedInfo.secret.isEmpty()) {
            editor.putBoolean("proxy_enabled_calls", false);
        }
        editor.apply();

        SharedConfig.currentProxy = selectedInfo;
        ProxyRuntimeStateStore.markConnectionStarting(selectedInfo, ProxyConnectionEvent.Origin.ROTATION_CANDIDATE);
        ...
        if ("fallback".equals(reason)) {
            log("switch fallback endpoint=" + endpoint(selectedInfo) + " ping=" + selectedInfo.ping);
        } else {
            log("switch fresh endpoint=" + endpoint(selectedInfo) + " ping=" + selectedInfo.ping);
        }
    }
