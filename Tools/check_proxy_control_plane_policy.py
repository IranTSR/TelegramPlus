...
def main() -> int:
    ...
    require_not(store, "public static boolean shouldScheduleFallback", "runtime store must not expose legacy fallback scheduling bypass", failures)
    ...
    require_not(scheduler, "markEndpointFailure", "ProxyCheckScheduler must not expose legacy live-failure bypass", failures)
    ...
    require(rotation, "reducerDecision", "ProxyRotationController must consume reducer decisions from stage notifications", failures)
    require(rotation, "rotationTrigger", "ProxyRotationController must schedule only reducer-confirmed rotation triggers", failures)
    require_not(rotation, "ProxyRuntimeStateStore.shouldScheduleFallback", "ProxyRotationController must not re-derive fallback scheduling outside reducer", failures)
    ...
    require(engine, "ProxyConnectionEvent.rotationTimeout", "ProxyRotationEngine must create a runtime event for connecting timeout", failures)
    require(engine, "ProxyRuntimeStateStore.onRuntimeEvent(event)", "ProxyRotationEngine must convert connecting timeout through the reducer", failures)
